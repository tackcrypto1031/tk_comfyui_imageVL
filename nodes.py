...
class TK_QwenVL_Interrogator_Single:
    def __init__(self):
        self.model = None
        self.processor = None
        self.current_model_id = None
    ...
    def interrogate(self, image, model_id, prompt, max_new_tokens, min_pixels, max_pixels, temperature, seed):
        # Set seed for reproducibility
        if seed is not None:
             torch.manual_seed(seed)
        ...
        # Load model if strictly necessary
        if self.model is None or self.current_model_id != model_id:
            print(f"Loading model from {model_path}...")
            from transformers import AutoModelForVision2Seq
        ...
        # Save to temp file because Qwen process_vision_info expects paths
...
class TK_JoyCaption_Interrogator_Single:
    def __init__(self):
        self.model = None
        self.processor = None
        self.tokenizer = None
    ...
    def interrogate(self, image, joycaption_model, caption_type, caption_length, user_prompt, max_new_tokens, temperature, top_p, top_k, cache_model):
        ...
        try:
            ...
            inputs = self.processor(text=text_prompt, images=run_image, return_tensors="pt")
            inputs = inputs.to(self.model.device)
